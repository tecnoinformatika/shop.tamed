...
class WINP_Helper {
	...
	public static function register_shortcode( $name, $obj ) {
		Wbcr_FactoryShortcodes325::register( $name, $obj );
	}
	...
	public static function get_factory_class() {
		return "factory-bootstrap-413 factory-fontawesome-000";
	}

	/**
	 * Wrapper for Wbcr_FactoryForms413_OptionsValueProvider object
	 *
	 * @param $plugin
	 *
	 * @return Wbcr_FactoryForms413_OptionsValueProvider
	 */
	public static function get_options_value_provider( $plugin ) {
		return new Wbcr_FactoryForms413_OptionsValueProvider( $plugin );
	}
	 ...
	 * @param $plugin
	 *
	 * @return Wbcr_FactoryForms413_Form
	 */
	public static function get_factory_form( $options, $plugin ) {
		return new Wbcr_FactoryForms413_Form( $options, $plugin );
	}
	...
	public static function register_factory_metaboxes( $class_name_or_object, $post_type, $plugin ) {
		Wbcr_FactoryMetaboxes405::registerFor( $class_name_or_object, $post_type, $plugin );
	}
	...
	public static function get_purchase_button() {
        ...
        <p class="winp-purchase-button">
			<?php $price_url = WINP_Plugin::app()->get_support()->get_pricing_url( true, 'library_page' ); ?>
			<?php $price_url = empty( $price_url ) ? 'http://woody-ad-snippets.webcraftic.com/?utm_content=library_page&utm_source=wordpress.org' : $price_url; ?>
            <a class="button" id="winp-library-buy-button" href="<?php echo esc_url( $price_url ) ?>" target="_blank">
				<?php $price = WINP_Plugin::app()->premium->get_price(); ?>
				<?php $price = empty( $price ) ? 19 : $price; ?>
