...
class WINP_ImportPage extends WINP_Page {
	...
	private function getMessage() {
		...
		$wbcr_import_error = WINP_Plugin::app()->request->request( 'wbcr_import_error', '' );
		...
		<?php } elseif ( intval( $wbcr_inp_imported ) >= 0 ) {
			...
			<?php
		} elseif ( ! empty( $wbcr_import_error ) ) { ?>
            <div id="message" class="alert alert-warning">
                <p>
                    <span class="dashicons dashicons-info"></span><?php echo sprintf( __( 'To import more then one snippet at a time, you need to purchase <a href="%s">Woody snippets PRO</a>', 'insert-php' ), 'http://woody-ad-snippets.webcraftic.com/' ); ?>
                </p>
            </div>
			<?php
		} elseif ( ! empty( $wbcr_saved ) ) { ?>
            <div id="message" class="alert alert-warning">
	...
	public function indexAction() {
		$wbcr_saved = WINP_Plugin::app()->request->post( $this->plugin->getPrefix() . 'saved', '' );
		if ( ! empty( $wbcr_saved ) ) {
			$wbcr_nonce = WINP_Plugin::app()->request->post( $this->plugin->getPrefix() . 'nonce', '' );
			if ( ! wp_verify_nonce( $wbcr_nonce, $this->plugin->getPrefix() . 'import_form' ) ) {
				wp_die( 'Permission error. You can not edit this page.' );
			}
		}

		$import_url = remove_query_arg( array( 'tab' ) );
                            ...
                            <div id="tab1">
                                ...
                                <div class="form-group form-horizontal">
                                    <div class="control-group controls col-sm-12">
                                        ...
                                        <input name="<?php echo $this->plugin->getPrefix() . 'saved' ?>" class="btn btn-primary" type="submit" value="<?php _e( 'Upload files and import', 'insert-php' ) ?>"/>
                                    </div>
                                </div>
                            </div>
                            <?php else: ?>
