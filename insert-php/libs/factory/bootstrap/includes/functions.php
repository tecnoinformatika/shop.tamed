...
add_action( 'wbcr_factory_bootstrap_413_plugin_created', 'wbcr_factory_bootstrap_413_plugin_created' );

/**
 * @param Wbcr_Factory413_Plugin $plugin
 */
function wbcr_factory_bootstrap_413_plugin_created( $plugin ) {
	$manager = new Wbcr_FactoryBootstrap413_Manager( $plugin );
	$plugin->setBootstap( $manager );
}

if ( ! class_exists( 'Wbcr_FactoryBootstrap413_Manager' ) ) {
	 ...
	 * @since 3.2.0
	 */
	class Wbcr_FactoryBootstrap413_Manager {
		...
		public function loadAssets( $hook ) {
			...
			do_action( 'wbcr_factory_413_bootstrap_enqueue_scripts', $hook );
			do_action( 'wbcr_factory_413_bootstrap_enqueue_scripts_' . $this->plugin->getPluginName(), $hook );
			...
			if ( $color_name !== 'fresh' ) {
				if ( file_exists( FACTORY_BOOTSTRAP_413_DIR . '/assets/flat/css/bootstrap.' . $color_name . '.css' ) ) {
					wp_enqueue_style( 'wbcr-factory-bootstrap-413-colors', FACTORY_BOOTSTRAP_413_URL . '/assets/flat/css/bootstrap.' . $color_name . '.css' );
				}
			}
            ...
            <script>
				...
				if( !window.factory.factoryBootstrap413 ) {
					window.factory.factoryBootstrap413 = {};
				}
				window.factory.factoryBootstrap413.colors = {
		...
		protected function enqueueScripts( array $sripts, $type = 'js', array $dependencies ) {
			...
			$cache_dir_path = FACTORY_BOOTSTRAP_413_DIR . '/assets/cache/';
			$cache_dir_url  = FACTORY_BOOTSTRAP_413_URL . '/assets/cache/';
			...
			if ( $cache_enable && file_exists( $cache_filepath ) ) {
				...
				foreach ( $sripts as $script_to_load ) {
					...
					if ( $cache_enable && $cache_dir_exists ) {
						$fname = FACTORY_BOOTSTRAP_413_DIR . "/assets/$type-min/$script_to_load.min." . $type;
						...
						if ( $type == 'js' ) {
							wp_enqueue_script( md5( $script_to_load ), FACTORY_BOOTSTRAP_413_URL . "/assets/$type-min/$script_to_load.min." . $type, $is_first ? $dependencies : false, $this->plugin->getPluginVersion() );
						} else {
							wp_enqueue_style( md5( $script_to_load ), FACTORY_BOOTSTRAP_413_URL . "/assets/$type-min/$script_to_load.min." . $type, [], $this->plugin->getPluginVersion() );
						}
						$is_first = false;
					}
				}
