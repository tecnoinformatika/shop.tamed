function UCAssetsManager(){
	...
	function getParentFolder(path){
		...
		var lastSap = path.lastIndexOf("/");
		
		if(lastSap == -1)
			lastSap = path.lastIndexOf("\\");
	...
	function deleteSelectedFiles(){
		...
		g_ucAdmin.ajaxRequest("assets_delete_files", {arrFiles: arrFiles, path: g_activePath, pathkey:g_pathKey}, function(response){
	...
	function initCreateFolderActions(){
		jQuery("#uc_dialog_create_folder_action").click(createFolder);
	...
	function initCreateFileActions(){
		...
		jQuery("#uc_dialog_create_file_action").click(createFile);
	...
	function onEditDialogOpen(item){
		...
		g_ucAdmin.ajaxRequest("assets_get_file_content", data, function(response){
	...
	function onEditDialogSave(){
		...
		g_ucAdmin.ajaxRequest("assets_save_file", data);
	...
	function dialogMoveFilesRequest(actionOnExists){
		...
		g_ucAdmin.ajaxRequest("assets_move_files", data, function(response){
	...
	function initMoveFileActions(){
		...
		//move files action
		jQuery("#uc_dialog_move_files_action").click(function(){
		...
		objDialogMove.find(".uc-dialog-move-message .unite-button-secondary").click(function(){
	...
	function initRenameFileActions(){
		...
		jQuery("#uc_dialog_rename_file_action").click(renameFile);
	...
	function unzipSelectedFile(){
		...
		g_ucAdmin.ajaxRequest("assets_unzip_file",data,function(response){
			objLoader.hide();
			g_objFileList.html(response.html);
	...
	function initActionsPanel(){
		 ...
		 */
		g_objPanel.find("a.uc-panel-button").click(function(){
	...
	function initEvents(){
		...
		g_objFileList.delegate("input.uc-filelist-checkbox", "click", onCheckboxClick);
		g_objFileList.delegate("a.uc-filelist-item", "click", onItemClick);
	...
	this.loadPath = function(file, byPath, quiteMode){
		...
		g_ucAdmin.ajaxRequest("assets_get_filelist", data , function(response){
