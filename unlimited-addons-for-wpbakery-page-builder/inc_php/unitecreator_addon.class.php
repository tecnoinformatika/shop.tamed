	...
	class UniteCreatorAddonWork extends UniteElementsBaseUC{
		...
		public function getHtmlConfig($putMode = false, $isOutputSidebar = false, $options = array()){
			...
			*/
			...
			if(!empty($this->params)){
				$objSettings->addSap(__("General",ADDONLIBRARY_TEXTDOMAIN),"config",true);
				$objSettings->initByCreatorParams($arrParams);
			}
			
			//add items
			if($this->hasItems){
				$objSettings->addSap(__("Items",ADDONLIBRARY_TEXTDOMAIN),"items");
				$objSettings->addItemsPanel($this);
			}
			...
			if($isFontsPanelEnabled == true){
				...
				$objSettings->addSap(__("Fonts",ADDONLIBRARY_TEXTDOMAIN),"fonts");
				...
				$objSettings->addFontPanel($arrParamNames, $arrFontsData,null,$fontsPanelOptions);
			}
			...
			if($numSettings == 0){
				$textEmpty = __("no settings for this addon", ADDONLIBRARY_TEXTDOMAIN);
